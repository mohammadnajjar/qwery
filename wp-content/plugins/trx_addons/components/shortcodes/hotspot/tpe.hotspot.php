	...
	#><div id="{{ id }}" class="<# print( trx_addons_apply_filters( 'trx_addons_filter_sc_classes', 'sc_hotspot sc_hotspot_' + settings.type + ( settings.class ? ' ' + settings.class : '' ), settings ) ); #>">
		...
		<div class="sc_hotspot_content sc_item_content">
			...
			_.each( settings.spots, function( item ) {
				...
				#><div class="sc_hotspot_item sc_hotspot_item_visible_{{ item.spot_visible }} sc_hotspot_item_open_{{ item.open }} {{ item.opened }}"
					...
					<div class="sc_hotspot_item_popup sc_hotspot_item_popup_<#
						print( item.position ? item.position : 'bc' );
					#>"><#
						...
						if ( item.source != 'custom' ) {
							item.image = { url: '' };
							item.subtitle = "<?php esc_html_e( 'Attention', 'trx_addons' ); ?>";
							item.title = "<?php esc_html_e( 'Dynamic content', 'trx_addons' ); ?>";
							item.description = "<?php esc_html_e( 'Dynamic content is not available in the edit mode!', 'trx_addons' ); ?>";
							item.price = '';
							item.link = { url: '' };
							item.link_text = '';
						}
