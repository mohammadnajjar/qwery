				...
				foreach ( $args['spots'] as $item ) {
					...
					if ( $item['source'] != 'custom' && (int)$item['post'] > 0 ) {
						...
						if ( ! empty( $post->ID ) ) {
							$GLOBALS['post'] = $post;
							setup_postdata( $post );
							$item['image'] = has_post_thumbnail() ? get_post_thumbnail_id( get_the_ID() ) : '';
							$item['subtitle'] = trx_addons_sc_show_post_meta( 'sc_hotspot', apply_filters( 'trx_addons_filter_post_meta_args', array(
													'components' => 'categories',
													'theme_specific' => false,
													'echo' => false
												), 'sc_hotspot_default' ) );
							$item['title'] = get_the_title();
							$item['price'] = apply_filters( 'trx_addons_filter_custom_meta_value', '', 'price' );
							$item['description'] = trx_addons_excerpt( get_the_excerpt(), apply_filters( 'trx_addons_filter_sc_hotspot_excerpt_length', 16 ) );
					...
					?><div class="sc_hotspot_item<?php
							echo " sc_hotspot_item_open_{$item['open']}"
					...
					?>>
						...
						<div class="sc_hotspot_item_popup sc_hotspot_item_popup_<?php
							echo esc_attr( ! empty( $item['position'] ) ? $item['position'] : 'bc' );
						?>"><?php
