...
if ( ! function_exists( 'trx_addons_sc_skills_load_scripts_front2' ) ) {
	add_action( 'wp_enqueue_scripts', 'trx_addons_sc_skills_load_scripts_front2', TRX_ADDONS_ENQUEUE_SCRIPTS_PRIORITY - 1 );
	function trx_addons_sc_skills_load_scripts_front2() {
		if ( trx_addons_is_off( trx_addons_get_option( 'optimize_css_and_js_loading' ) ) ) {
			wp_enqueue_script( 'chart', trx_addons_get_file_url(TRX_ADDONS_PLUGIN_SHORTCODES . 'skills/chart.min.js'), array('jquery'), null, true );
		}
	}
}
...
if ( !function_exists( 'trx_addons_sc_skills_load_scripts_front' ) ) {
	...
	function trx_addons_sc_skills_load_scripts_front( $force = false ) {
		...
		if ( ! $loaded && ! $preview_gb && ( ( ! $optimize && $debug ) || ( $optimize && $need ) ) ) {
			...
			do_action( 'trx_addons_action_load_scripts_front', $force, 'sc_skills' );
		}
		if ( ! $loaded && $preview_elm && $optimize && ! $debug && ! $theme_full ) {
			do_action( 'trx_addons_action_load_scripts_front', false, 'sc_skills', 2 );
		}
	}
}
...
if ( ! function_exists( 'trx_addons_sc_skills_load_scripts_front_responsive' ) ) {
	...
	function trx_addons_sc_skills_load_scripts_front_responsive( $force = false ) {
		static $loaded = false;
		if ( ! $loaded && (
			current_action() == 'wp_enqueue_scripts' && trx_addons_need_frontend_scripts( 'sc_skills' )
			||
			current_action() != 'wp_enqueue_scripts' && $force === true
			)
		) {
			$loaded = true;
			wp_enqueue_style( 'trx_addons-sc_skills-responsive', trx_addons_get_file_url(TRX_ADDONS_PLUGIN_SHORTCODES . 'skills/skills.responsive.css'), array(), null, trx_addons_media_for_load_css_responsive( 'sc-skills', 'md' ) );
		}
	}
}
...
if ( !function_exists( 'trx_addons_sc_skills_merge_scripts' ) ) {
	...
	function trx_addons_sc_skills_merge_scripts($list) {
		//$list[ TRX_ADDONS_PLUGIN_SHORTCODES . 'skills/chart.min.js' ] = false;
...
//	add_filter( 'trx_addons_filter_get_menu_cache_html', 'trx_addons_sc_skills_check_in_html_output', 10, 1 );
	...
	function trx_addons_sc_skills_check_in_html_output( $content = '' ) {
		if ( ! trx_addons_need_frontend_scripts( 'sc_skills' )
