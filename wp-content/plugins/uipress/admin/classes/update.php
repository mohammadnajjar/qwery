...
class uipress_update
{
  public function __construct($version, $pluginName, $pluginPath, $textDomain, $pluginURL)
  {
    ...
    $this->updateURL = "https://api.uipress.co/latest/";
  ...
  public function update_a2020_settings()
  {
    ...
    <div class="notice update-message uip-border-round uip-no-border uip-background-primary-wash uip-padding-s" id="uip-automatic-settings-importer">
      <div class="uip-text-l uip-text-bold uip-text-emphasis uip-margin-bottom-xs">
        <span><?php echo __("Welcome to UiPress version ", "uipress") . $this->version; ?></span>
        <span class="uip-margin-left-xs">🎉</span>
      </div>
      <div class="uip-text-normal uip-margin-bottom-m"><?php _e("Would you like to automatically import settings from your older version of uipress?", "uipress"); ?></div>
      <div class="">
        <button class="uip-button-default uip-margin-right-s" onclick="hideImportSettings()"><?php _e("Don't show again", "uipress"); ?></button>
        <button class="uip-button-secondary" onclick="importOldSettings('<?php echo $network; ?>')"><?php _e("Import settings", "uipress"); ?></button>
      </div>
    </div>
    <?php
  }
  ...
  public function update_a2020_settings_menu()
  {
    ...
    <div class="uip-border-round uip-no-border uip-background-primary-wash uip-padding-s uip-margin-top-m" id="uip-automatic-settings-importer">
      <div class="uip-text-normal uip-margin-bottom-s"><?php _e("Would you like to automatically import settings from your older version of uipress?", "uipress"); ?></div>
      <div class="">
        <button class="uip-button-secondary" onclick="importOldSettings('<?php echo $network; ?>')"><?php _e("Import settings", "uipress"); ?></button>
      </div>
    </div>
  ...
  public function add_settings_link($plugin_meta, $plugin_file_name)
  {
    if ($plugin_file_name == "uipress/uipress.php") {
      $plugin_meta[] = '<a href="#" id="update-uip" onclick="uip_check_for_updates()">' . __("Check for updates", "uipress") . "</a>";
    }
    return $plugin_meta;
  }
  ...
  public function uip_check_for_updates()
  {
    if (defined("DOING_AJAX") && DOING_AJAX && check_ajax_referer("uip-security-nonce", "security") > 0) {
      ...
      if ($this->is_response_clean($remote)) {
        ...
        $message = __("Unable to contact update server", "uipress");
      ...
      if ($remote) {
        ...
        if ($body && version_compare($this->version, $body->version, "<")) {
          ...
          $returndata["message"] = __("Update available", "uipress");
          echo json_encode($returndata);
          die();
        }
      }

      $message = __("No Updates available", "uipress");
      $returndata["error"] = true;
      $returndata["message"] = $message;
      echo json_encode($returndata);
      die();
  ...
  public function uip_hide_import_old_settings()
  {
    if (defined("DOING_AJAX") && DOING_AJAX && check_ajax_referer("uip-security-nonce", "security") > 0) {
      ...
      $returndata["success"] = true;
      $returndata["message"] = __("Message dismissed", "uipress");
      echo json_encode($returndata);
      die();
    }
  }
  ...
  public function uip_import_old_settings()
  {
    if (defined("DOING_AJAX") && DOING_AJAX && check_ajax_referer("uip-security-nonce", "security") > 0) {
      ...
      if (!$oldSettings || !is_array($oldSettings)) {
        $message = __("No settings to import", "uipress");
        $returndata["error"] = true;
        $returndata["message"] = $message;
        echo json_encode($returndata);
        die();
      ...
      $returndata["success"] = true;
      $returndata["message"] = __("Settings imported", "uipress");
      echo json_encode($returndata);
      die();
    }
  }
